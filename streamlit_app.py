...
with tab1:
    ...
    # 1. 住所検索エリア
    st.markdown("##### 📍 住んでいる地域の設定")
    col_zip, col_btn, col_res = st.columns([2, 1, 3])
    with col_zip:
        zipcode_in = st.text_input("郵便番号 (7桁・ハイフンなし)", max_chars=7, placeholder="1000001", key="zip_input")
    with col_btn:
        st.write("") 
        st.write("") 
        if st.button("住所検索", key="search_btn"):
            pref, addr = get_address_from_zip(zipcode_in)
            if pref:
    ...
    with col_res:
        st.write("")
        st.write("")
        current_addr = st.session_state.get("address_found", "")
        st.text_input("住所", value=current_addr, disabled=True, label_visibility="collapsed")
    ...
    if calc_submitted:
        pref_to_use = st.session_state.get("prefecture_found", "その他")
